    ...
    argumentLength:
      typeof toolCall?.function?.arguments === 'string'
        ...
        : toolCall?.function?.arguments
          ? JSON.stringify(toolCall.function.arguments).length
          : 0,
  }));
}
    ...
    for (const frame of frames) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          console.warn('[agent-api/minimax] failed to parse SSE chunk', {
...
export function createMiniMaxService() {
  async function requestChatCompletion({ messages, tools, temperature = 1 }) {
    const env = getEnv();
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
      ...
      console.log('[agent-api/minimax] request:start', {
        attempt: attempt + 1,
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log('[agent-api/minimax] request:success', {
            attempt: attempt + 1,
        ...
        if (isRetryable && attempt < MAX_RETRIES) {
          ...
          console.warn('[agent-api/minimax] retrying request', {
            attempt: attempt + 1,
        ...
        if (isRetryable) {
          const delayMs = getRetryDelay(attempt);
          console.warn('[agent-api/minimax] retrying after request error', {
            attempt: attempt + 1,
            delayMs,
            message: error instanceof Error ? error.message : String(error),
          });
  ...
  return {
    ...
    async streamText({ system, user, fallbackText, temperature = 1, onTextDelta }) {
      const env = getEnv();
      ...
      console.log('[agent-api/minimax] stream:start', {
        url: `${env.minimaxBaseUrl}/chat/completions`,
        model: env.minimaxModel,
        temperature,
      });
      ...
      try {
        ...
        if (!response.ok || !response.body) {
          const errorText = await response.text();
          console.warn('[agent-api/minimax] stream failed', {
            status: response.status,
            body: errorText.slice(0, 500),
          });
        ...
        console.log('[agent-api/minimax] stream:success', {
          contentLength: emittedText.length,
        });

        return emittedText || fallbackText;
      } catch (error) {
        console.warn('[agent-api/minimax] stream error', {
          message: error instanceof Error ? error.message : String(error),
        });
    ...
    async debugChatCompletion({ messages, tools, temperature = 0.2 }) {
    ...
    async createMessage({ messages, tools, temperature = 0.2 }) {
      const payload = await requestChatCompletion({
        messages,
        tools,
        temperature,
      ...
      const message = payload?.choices?.[0]?.message;
      if (!message) {
        console.warn('[agent-api/minimax] missing message response', payload);
        return null;
      }

      return {
        ...message,
        content: typeof message.content === 'string' ? stripThinkTags(message.content) : message.content,
      };
    },
  };
}
