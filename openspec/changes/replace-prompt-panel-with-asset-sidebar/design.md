...
- 让左侧素材栏、中间画布工作区与右侧对话栏在纵向上铺满 header 下方的全部可用高度。
...
### 1.1 三栏需要纵向铺满工作区，而不是为底部悬浮控件预留固定空白

工作区容器应在 header 下方占满剩余可用高度，左侧素材栏、中间画布工作区和右侧对话栏都需要随之拉伸到同一纵向高度。底部悬浮控件应继续浮在中间工作区内部，而不是通过外层容器的额外底部 padding 预留空间。

Why:
- 固定的外层底部留白会让三栏看起来没有铺满页面，破坏“正式工作区”感。
- `FloatingFooter` 和 `ToolDock` 本身已经是覆盖在中间工作区上的浮层，没有必要通过压缩整体高度为它们腾位置。
- 让三栏跟随工作区高度拉伸，可以让左中右三块视觉边界保持齐平。

Alternative considered:
- 继续保留外层较大的底部 padding，以避免悬浮控件贴近视口底部。
Rejected because 这会直接造成工作区底部空白，并让左右侧栏和中间画布都显得“悬空”。
...
- [为了给底部悬浮控件留空间而给外层容器增加固定底部留白，会破坏全高布局] → 将底部控件继续保留在中间工作区覆盖层中，改用工作区内部定位而不是压缩整体高度。
