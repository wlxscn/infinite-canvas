...
- [未来若要支持自由端点、折线路由，当前直线模型可能需要扩展] → 预留 `attached | free` 端点结构，并把路径计算收敛在 connector adapter 内部。
