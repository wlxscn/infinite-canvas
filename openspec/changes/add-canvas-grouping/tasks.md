...
- [ ] 1.4 扩展选择状态与相关类型，使同一上下文内可维护多个已选节点，并为成组提供稳定的多选输入。
...
- [ ] 3.4 在画布交互层补充与成组直接相关的多选输入路径，例如追加选择和同一上下文内的多节点成组触发。
...
- [ ] 4.4 调整工具栏与选择反馈，使多选时可见“成组”入口，并在创建 group 后切换为单个 group 选中态。
...
- [ ] 5.4 为多元素成组补充单元测试和 E2E，覆盖多选成组、创建后选择收敛以及不同行为上下文的边界。
