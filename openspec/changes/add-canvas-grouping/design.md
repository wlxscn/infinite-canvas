...
- `apps/web/src/state/store.ts`、选择相关 hooks / toolbar
  需要把当前单选状态扩展为足以支撑多元素成组的多选状态。
...
- 支持在同一编辑上下文中选中多个支持节点，并一次创建包含这些节点的 group。
...
- 不引入完整多选系统的大规模重构；第一版只扩展到足以支撑多元素成组创建、组选择切换与组内编辑。
...
- 当前 `selectedId` 需要扩展为可表示多个已选节点的选择状态。
- 成组操作基于当前上下文内的多选集合创建 group，而不是只包裹单个节点。
- 当创建 group 成功后，选择状态收敛为新 group 的整体选中态。

原因：

- 没有多选集合就无法支撑“多个元素一次成组”。
- 成组前后的选择收敛规则明确，才能保持工具栏和历史语义稳定。

备选方案：

- 继续只保留单选并通过重复操作拼接 group：不符合成组的基本用户预期。
- 直接引入完整框选、批量属性编辑、多选变换：范围过大，不适合作为这次补充。

### 5. connector 继续附着具体子节点，group 只参与几何传递
...
### 6. 成组/拆分组作为文档 mutation，进入/退出组作为导航状态
...
- [多选状态进入后，现有单选工具栏和删除/层级操作会受影响] → 把多选范围收敛到与成组直接相关的路径，并明确“多选集合 -> 创建 group -> 单个 group 选中”的转换规则。
